 ...
 */
/*global importClass, java, Packages, attributes, elements, FileUtils, InputStreamReader, FileInputStream, File, self, OutputStreamWriter, FileOutputStream, project, readFile, print*/
if (typeof self !== 'undefined' && typeof self.getTaskName !== 'undefined') {
	var ant = function () {
		...
		var defines = attributes.get('defines'),
			...
			inputcharset = elements.get('inputcharset') || 'utf-8',
			outputcharset = elements.get('outputcharset') || 'utf-8',
			mapping = [], i = 0, j = 0, l = 0, fileset, files = [], def = {},
			fileToString = function (file) {
				return '' + FileUtils.readFully(new InputStreamReader(new FileInputStream(file), inputcharset)).toString();
			},
			out, result = [];
		...
		if (file) {
			if (tofile || todir) {
				...
				if(!srcFile.exists()) {
					srcFile = new File(project.getBaseDir(), file);
				}
				var filename = file.substring(file.lastIndexOf('/')+1);
				var dstFile = new File(tofile || (todir+"/"+filename));
				if(!dstFile.exists()) {
					dstFile = new File(project.getBaseDir(), tofile || (todir+"/"+filename));
				}
			...
			if (todir) {
				var toDirFile = new File(todir);
				if(!toDirFile.exists()) {
					toDirFile = new File(project.getBaseDir(), todir);
				}
				...
				for (; i < filesets.size(); i += 1) {
					...
					for (; j < files.length; j += 1) {
						var dstFile = new File(toDirFile, files[j]);
						var srcFile = new File(files[j]);
						if(!srcFile.exists()) {
							srcFile = new File(fileset.getDir(project), files[j]);
						}
		...
		for (i = 0; i < mapping.length; i += 1) {
			...
			out = new OutputStreamWriter(new FileOutputStream(mapping[i].dst), outputcharset);
